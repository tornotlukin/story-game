...
init python:
    ...
    # Gaussian Blur 9-tap - Ported from Phaser blur-med.js
    ...
    """, vertex_300="""
        v_tex_coord = a_tex_coord;
    """, fragment_300="""
        // Ported from Phaser 9-tap Gaussian blur
        ...
        vec4 col = vec4(0.0);
        col += texture2D(tex0, clamp(v_tex_coord, 0.0, 1.0)) * 0.2270270270;
        col += texture2D(tex0, clamp(v_tex_coord + (off1 / resolution), 0.0, 1.0)) * 0.3162162162;
        col += texture2D(tex0, clamp(v_tex_coord - (off1 / resolution), 0.0, 1.0)) * 0.3162162162;
        col += texture2D(tex0, clamp(v_tex_coord + (off2 / resolution), 0.0, 1.0)) * 0.0702702703;
        col += texture2D(tex0, clamp(v_tex_coord - (off2 / resolution), 0.0, 1.0)) * 0.0702702703;

        gl_FragColor = col;
    """)

    # Gaussian Blur 13-tap - Ported from Phaser blur-high.js
    ...
    """, vertex_300="""
        v_tex_coord = a_tex_coord;
    """, fragment_300="""
        // Ported from Phaser 13-tap Gaussian blur
        vec2 resolution = u_model_size;
        ...
        vec4 col = vec4(0.0);
        col += texture2D(tex0, clamp(v_tex_coord, 0.0, 1.0)) * 0.1964825501511404;
        col += texture2D(tex0, clamp(v_tex_coord + (off1 / resolution), 0.0, 1.0)) * 0.2969069646728344;
        col += texture2D(tex0, clamp(v_tex_coord - (off1 / resolution), 0.0, 1.0)) * 0.2969069646728344;
        col += texture2D(tex0, clamp(v_tex_coord + (off2 / resolution), 0.0, 1.0)) * 0.09447039785044732;
        col += texture2D(tex0, clamp(v_tex_coord - (off2 / resolution), 0.0, 1.0)) * 0.09447039785044732;
        col += texture2D(tex0, clamp(v_tex_coord + (off3 / resolution), 0.0, 1.0)) * 0.010381362401148057;
        col += texture2D(tex0, clamp(v_tex_coord - (off3 / resolution), 0.0, 1.0)) * 0.010381362401148057;

        gl_FragColor = col;
    """)

    # Simple box blur (faster, lower quality)
    ...
    """, fragment_300="""
        ...
        for (float x = -3.0; x <= 3.0; x += 1.0) {
            for (float y = -3.0; y <= 3.0; y += 1.0) {
                ...
                col += texture2D(tex0, sampleCoord);
                count += 1.0;
            }
        }
