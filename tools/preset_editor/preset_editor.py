...
def get_all_text_shaders() -> List[str]:
    ...
    custom = app.text_shader_parser.list_available_text_shaders()
    for name in custom:
...
def refresh_textshader_ui():
    ...
    if app.textshader_mode == EditorMode.MANAGER:
        refresh_textshader_manager()
    elif app.textshader_mode == EditorMode.BUILDER:
        refresh_textshader_builder()
    elif app.textshader_mode == EditorMode.JSON:
...
def refresh_textshader_manager():
    ...
    selected = app.textshader_selection.selected
...
def textshader_manager_select_callback(sender, app_data, user_data):
    """Callback for text shader manager mode selection."""
...
def textshader_manager_select(name: str):
    ...
    ctrl = dpg.is_key_down(dpg.mvKey_LControl) or dpg.is_key_down(dpg.mvKey_RControl)
    shift = dpg.is_key_down(dpg.mvKey_LShift) or dpg.is_key_down(dpg.mvKey_RShift)
    app.textshader_selection.handle_click(name, ctrl, shift)
    refresh_textshader_manager()


def textshader_move_selected_top():
    for name in reversed(app.textshader_selection.selected):
        app.json_mgr.move_textshader(name, "top")
    refresh_textshader_manager()


def textshader_move_selected_up():
    for name in app.textshader_selection.selected:
        app.json_mgr.move_textshader(name, "up")
    refresh_textshader_manager()


def textshader_move_selected_down():
    for name in reversed(app.textshader_selection.selected):
        app.json_mgr.move_textshader(name, "down")
    refresh_textshader_manager()


def textshader_move_selected_bottom():
    for name in app.textshader_selection.selected:
        app.json_mgr.move_textshader(name, "bottom")
    refresh_textshader_manager()


def textshader_select_all():
    app.textshader_selection.select_all()
    refresh_textshader_manager()


def textshader_select_none():
    app.textshader_selection.select_none()
    refresh_textshader_manager()


def textshader_invert_selection():
    app.textshader_selection.invert_selection()
    refresh_textshader_manager()


def textshader_delete_selected():
    selected = app.textshader_selection.selected.copy()
    if not selected:
        return

    def do_delete():
        app.json_mgr.delete_textshaders(selected)
...
def textshader_duplicate_selected():
    selected = app.textshader_selection.selected.copy()
    for name in selected:
        new_name = app.json_mgr.get_unique_textshader_name(f"{name}_copy")
        app.json_mgr.duplicate_textshader(name, new_name)
    refresh_all()
...
def textshader_builder_create_new():
    ...
    # Determine shader value (None if "(none)" selected)
    shader_value = source if source != "(none)" else None

    # Generate unique name based on shader
    if shader_value:
        base_name = shader_value
    else:
        base_name = "text_style"
    new_name = app.json_mgr.get_unique_textshader_name(base_name)
    ...
    if shader_value and shader_value not in BUILTIN_TEXT_SHADERS:
        # Custom text shader - get params from parser
        shader_def = app.text_shader_parser.get_text_shader(shader_value)
        if shader_def:
            for param in shader_def.params:
                if param.default is not None:
    ...
    # Create preset with default text styling
    preset_data = {
        "shader": shader_value,
    ...
    app.json_mgr.add_textshader(new_name, preset_data)
    app.textshader_selection.selected = [new_name]
    refresh_textshader_builder()


def refresh_textshader_builder():
    """Refresh the text shader builder panel (list + content)."""
    # Update the available text shaders combo (built-in + custom)
...
def textshader_builder_select_callback(sender, app_data, user_data):
    """Callback for text shader builder mode selection."""
...
def refresh_textshader_builder_list():
    ...
    presets = app.json_mgr.get_textshader_names()
    for name in presets:
        is_selected = name in app.textshader_selection.selected
        prefix = "[*] " if is_selected else "    "
        item_id = dpg.add_selectable(
            label=f"{prefix}{name}",
            default_value=is_selected,
...
def textshader_builder_select(name: str):
    """Select a text shader preset in builder mode."""
    ctrl = dpg.is_key_down(dpg.mvKey_LControl) or dpg.is_key_down(dpg.mvKey_RControl)
    if ctrl:
        if name in app.textshader_selection.selected:
            app.textshader_selection.selected.remove(name)
        else:
            app.textshader_selection.selected.append(name)
    else:
        app.textshader_selection.selected = [name]
    refresh_textshader_builder_list()
    refresh_textshader_builder_content()


def textshader_update_name_button_callback(sender, app_data, user_data):
    """Callback for Update Name button in text shader builder."""
    old_name, input_tag = user_data
    new_name = dpg.get_value(input_tag)
...
def textshader_rename_callback(sender, app_data, user_data):
    """DearPyGui callback for text shader rename input."""
    if user_data:
...
def refresh_textshader_builder_content():
    ...
    name = selected[0]
    preset = app.json_mgr.get_textshader(name)
    if not preset:
        dpg.add_text(f"Preset '{name}' not found",
                    parent="textshader_builder_content")
        return
...
def textshader_text_callback(sender, app_data, user_data):
    """Callback for text property inputs."""
    if user_data:
        name, prop = user_data
        preset = app.json_mgr.get_textshader(name) or {}
...
def textshader_text_bool_callback(sender, app_data, user_data):
    """Callback for text boolean property inputs."""
    if user_data:
        name, prop = user_data
        preset = app.json_mgr.get_textshader(name) or {}
        if "text" not in preset:
...
def textshader_outline_callback(sender, app_data, user_data):
    """Callback for outline property inputs."""
    if user_data:
...
def textshader_outline_color_callback(sender, app_data, user_data):
    """Callback for outline color input."""
    if user_data:
...
def textshader_rename_preset(old_name: str, new_name: str):
    ...
    if not new_name or new_name == old_name:
        return
    if app.json_mgr.get_textshader(new_name):
        return
    preset = app.json_mgr.get_textshader(old_name)
    if preset:
        app.json_mgr.set_textshader(new_name, preset)
        app.json_mgr.delete_textshader(old_name)
        app.textshader_selection.clear()
        app.textshader_selection.toggle(new_name)
        refresh_all()
        update_status_bar()
...
def switch_textshader_mode(mode: EditorMode):
    app.textshader_mode = mode

    if dpg.does_item_exist("textshader_mode_builder"):
        dpg.set_value("textshader_mode_builder", mode == EditorMode.BUILDER)
    if dpg.does_item_exist("textshader_mode_manager"):
        dpg.set_value("textshader_mode_manager", mode == EditorMode.MANAGER)
    if dpg.does_item_exist("textshader_mode_json"):
        dpg.set_value("textshader_mode_json", mode == EditorMode.JSON)

    if dpg.does_item_exist("textshader_builder_panel"):
        dpg.configure_item("textshader_builder_panel", show=(mode == EditorMode.BUILDER))
    if dpg.does_item_exist("textshader_manager_panel"):
        dpg.configure_item("textshader_manager_panel", show=(mode == EditorMode.MANAGER))
    if dpg.does_item_exist("textshader_json_panel"):
        dpg.configure_item("textshader_json_panel", show=(mode == EditorMode.JSON))
...
def add_new_textshader():
    name = app.json_mgr.get_unique_textshader_name("new_text_preset")
    app.json_mgr.add_textshader(name, {
        "shader": None,
        "shader_params": {},
        "text": {
            "font": "DejaVuSans.ttf",
...
def setup_ui():
    ...
    with dpg.window(tag="primary_window"):
        ...
        with dpg.tab_bar():
            ...
            # ========== TEXT SHADERS TAB ==========
            with dpg.tab(label="TEXT SHADERS"):
                with dpg.group(horizontal=True):
                    dpg.add_text("Mode:")
                    dpg.add_radio_button(
                        items=["Builder", "Manager", "JSON"],
                        default_value="Manager",
                        horizontal=True,
                        callback=lambda s, a: switch_textshader_mode(EditorMode[a.upper()])
                    )
                    dpg.add_spacer(width=30)
