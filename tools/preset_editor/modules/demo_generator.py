...
class DemoItem:
    ...
    Each item has a target that determines how it renders:
    - "character": Uses character artwork with transitions/shaders
    - "dialog": Uses dialog box with text shaders
      - use_dialog_background=True: Custom dialog artwork
      - use_dialog_background=False: Default say screen (black rect)
    """
    transition: Optional[str] = None
    shader: Optional[str] = None
    text_shader: Optional[str] = None
    # Target determines rendering mode: "character" or "dialog"
    target: str = "character"
    # Whether to use custom dialog background artwork (only for dialog target)
    use_dialog_background: bool = False
...
class DemoGenerator:
    ...
    def add_item(
        ...
        Args:
            transition: Transition preset name
            shader: Shader preset name
            text_shader: Text shader preset name
            target: Rendering target - "character" or "dialog"
            use_dialog_background: Whether to use custom dialog artwork (dialog target only)
        ...
        item = DemoItem(
            ...
            target=target,
            use_dialog_background=use_dialog_background
        )
    ...
    def generate_script(self) -> str:
        ...
        - Dialog: Shader on dialog artwork + text shader on text
        - Text shaders: {shader=...} tags in dialogue string

        Each item has its own target ("character" or "dialog") that determines
        how it renders independently.
        ...
        # Count targets for header comment
        char_count = sum(1 for item in self.items if item.target == "character")
        dialog_count = sum(1 for item in self.items if item.target == "dialog")
        mode_str = f"{char_count} character, {dialog_count} dialog"

        lines = [
            "## preset_demo.rpy - Auto-generated preset demo",
            "## Generated by Preset Editor",
            "##",
            f"## Testing {len(self.items)} preset combinations ({mode_str})",
            "",
        ]
        ...
        for i, item in enumerate(self.items):
            ...
            if item.target == "dialog":
                ...
                # Set dialog background to artwork (only if use_dialog_background is True)
                ...
                # Character Mode (target == "character")
